...
async function deployToTarget(target, environment) {
  ...
  switch (target) {
    case 'railway':
      ...
      await runCommand(`${config.command} ${config.service}`, `Railway deployment to ${environment}`);
      break;
      
    case 'heroku':
      // Ensure heroku remote exists
      try {
        await runCommand(`heroku git:remote -a ${config.app}`, `Setting up Heroku remote for ${environment}`);
      } catch (error) {
    ...
    case 'docker':
      await runCommand(config.command, `Docker build and push for ${environment}`);
      break;
...
async function postDeploymentTasks(target, environment) {
  ...
  if (process.env.DATABASE_PROVIDER === 'firebase') {
    try {
      await runCommand(`node scripts/deploy-firebase-rules.js ${environment}`, 'Deploying Firebase rules');
    } catch (error) {
...
async function deploy(target, environment, options = {}) {
  try {
    ...
    console.log(`   Git commit: ${execSync('git rev-parse HEAD').toString().trim()}`);
...
if (!deploymentTargets[target]) {
  ...
  console.log(`Available targets: ${Object.keys(deploymentTargets).join(', ')}`);
  process.exit(1);
}
