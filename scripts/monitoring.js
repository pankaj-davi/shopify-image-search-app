...
const monitoringConfig = {
  endpoints: [
    {
      name: 'Health Check',
      url: process.env.APP_URL ? `${process.env.APP_URL}/health` : 'http://localhost:3000/health',
      timeout: 5000,
      expectedStatus: 200
    },
    {
      name: 'API Status',
      url: process.env.APP_URL ? `${process.env.APP_URL}/api/health` : 'http://localhost:3000/api/health', 
      timeout: 5000,
      expectedStatus: 200
    }
  ],
...
class HealthMonitor {
  ...
  generateAlerts() {
    ...
    if (failedEndpoints.length > 0) {
      alerts.push({
        ...
        details: failedEndpoints.map(r => `${r.name}: ${r.error || `Status ${r.status}`}`)
      });
    }
    
    // Check response times
    const slowEndpoints = this.results.filter(r => 
      r.responseTime && r.responseTime > monitoringConfig.thresholds.responseTime
    );
  ...
  async sendSlackNotification(report) {
    try {
      const color = report.alerts.some(a => a.level === 'critical') ? '#ff0000' : '#ffa500';
      const payload = {
  ...
  async sendDiscordNotification(report) {
    try {
      const color = report.alerts.some(a => a.level === 'critical') ? 16711680 : 16753920; // Red or Orange
      const payload = {
        embeds: [{
